...
class TestChatApp:
    ...
    def setup_env(self):
        ...
        os.environ["APIGW_REST_INVOKE_URL"] = "http://api.test"
        os.environ["WS_BASE_URL"] = "ws://ws.test"
        os.environ["API_ENDPOINT_SSM_PARAM"] = ""
        os.environ["API_ID_SSM_PARAM"] = ""
        yield
        if "STREAMLIT_TESTING" in os.environ:
            del os.environ["STREAMLIT_TESTING"]
        if "APIGW_REST_INVOKE_URL" in os.environ:
            del os.environ["APIGW_REST_INVOKE_URL"]
        if "WS_BASE_URL" in os.environ:
            del os.environ["WS_BASE_URL"]
        if "API_ENDPOINT_SSM_PARAM" in os.environ:
            del os.environ["API_ENDPOINT_SSM_PARAM"]
        if "API_ID_SSM_PARAM" in os.environ:
            del os.environ["API_ID_SSM_PARAM"]
    ...
    def test_change_conversation(self, mock_api, mock_ws):
        ...
        assert at.session_state.channel_id == "room-2"
