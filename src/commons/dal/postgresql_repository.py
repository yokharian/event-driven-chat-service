...
class PostgreSQLRepository(IRepository):
    ...
    def _execute_update(self, query: str, params: Optional[Dict[str, Any]] = None) -> None:
    ...
    def update(self, params: Dict[str, Any], **keys) -> None:
        ...
        """
        if not keys:
            raise ValueError("At least one key must be provided to identify the item to update")

        # Filter out primary key from update params
        update_params = {
            k: self._serialize_value(v) for k, v in params.items() if k != self.primary_key
        }
